...
function ChannelModal({ members, onClose, onSave, onError }: { members: TeamMember[]; onClose: () => void; onSave: (input: { name?: string; type: "GROUP" | "DIRECT"; memberIds: string[] }) => Promise<void>; onError: (reason: unknown) => void }) {
  ...
  function choose(memberId: string) {
    setSelected((current) => {
      if (kind === "DIRECT") return new Set([memberId]);
      const next = new Set(current);
...
function ChannelMembersModal({ channel, members, onClose, onSave, onError }: { channel: Channel; members: TeamMember[]; onClose: () => void; onSave: (memberIds: string[]) => Promise<void>; onError: (reason: unknown) => void }) {
  const [selected, setSelected] = useState<Set<string>>(() => new Set(channel.members.map((member) => member.id).filter((id) => members.some((candidate) => candidate.id === id))));
  const [busy, setBusy] = useState(false);
  const [localError, setLocalError] = useState("");
  function toggle(id: string) { setSelected((current) => { const next = new Set(current); if (next.has(id)) next.delete(id); else next.add(id); return next; }); }
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!selected.size) { setLocalError("Mantenha pelo menos um colaborador no grupo"); return; }
    setBusy(true); setLocalError("");
    try { await onSave([...selected]); }
    catch (reason) { setLocalError(reason instanceof Error ? reason.message : "Não foi possível atualizar o grupo"); onError(reason); }
    finally { setBusy(false); }
  }
  return <div className="modal-backdrop"><section className="modal modal-small conversation-modal"><header><div><span className="section-kicker">PARTICIPANTES DO GRUPO</span><h2>{channel.name}</h2><p>Adicione ou remova colaboradores com acesso ativo.</p></div><button onClick={onClose} aria-label="Fechar"><X /></button></header><form onSubmit={submit}><div className="participant-selector"><div><span>Colaboradores</span><em>{selected.size} selecionado{selected.size === 1 ? "" : "s"}</em></div><div className="participant-list">{members.map((member) => { const checked = selected.has(member.id); return <button type="button" className={checked ? "selected" : ""} onClick={() => toggle(member.id)} key={member.id} aria-pressed={checked}><span className="avatar avatar-fallback">{initials(member.name)}</span><span><strong>{member.name}</strong><small>{member.jobTitle}</small></span><i>{checked ? <Check /> : <Plus />}</i></button>; })}</div></div>{localError && <div className="form-error modal-error"><AlertTriangle />{localError}</div>}<div className="modal-note"><LockKeyhole /> Pessoas removidas deixam de acessar o histórico deste grupo imediatamente.</div><footer><button type="button" className="button button-soft" onClick={onClose}>Cancelar</button><button className="button button-primary" disabled={busy}>{busy && <Loader2 className="spin" />}Salvar participantes</button></footer></form></section></div>;
}
...
function Empty({ icon: Icon, title, text, action, onAction }: { icon: typeof CalendarDays; title: string; text: string; action?: string; onAction?: () => void }) {
  return <div className="empty-state"><span><Icon /></span><h3>{title}</h3><p>{text}</p>{action && onAction && <button className="button button-primary empty-action" onClick={onAction}>{action}<ArrowRight /></button>}</div>;
}
